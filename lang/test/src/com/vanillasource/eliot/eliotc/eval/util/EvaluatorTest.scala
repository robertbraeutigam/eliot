...
class EvaluatorTest extends ProcessorTest() {
  ...
  it should "report unknown parameter error at call site in evaluateValueToNormalForm" in {
  ...
  private def runNormalFormForErrors(
    ...
    for {
      generator <- createGenerator(Seq(bigIntTypeFact, stringTypeFact) ++ facts)
      result    <- Evaluator.evaluateValueToNormalForm(vfqn, callSiteSourced).run(generator).run(Chain.empty).value
    } yield result match {
  ...
  private def runNormalFormDirectForErrors(
    ...
    for {
      generator <- createGenerator(Seq(bigIntTypeFact, stringTypeFact) ++ facts)
      result    <- Evaluator.evaluateToNormalForm(expression).run(generator).run(Chain.empty).value
    } yield result match {
      case Left(errors)                          => errors.toList
      case Right((errors, _)) if errors.nonEmpty => errors.toList
      case _                                     => throw new Exception("Expected error but evaluation succeeded")
