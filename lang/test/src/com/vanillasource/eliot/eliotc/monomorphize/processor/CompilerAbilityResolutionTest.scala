...
class CompilerAbilityResolutionTest extends ProcessorTest(LangProcessors(systemModules = Seq.empty)*) {
    ...
    compilerScan(Seq("eliot", "compiler"), "Type", "type Type") ++
      ...
      compilerScan(
        ...
        """import eliot.lang.Function
          ...
          |
          |def foldEither[E, A, B](ifLeft: Function[E, B], ifRight: Function[A, B], e: Either[E, A]): B = e match {
          ...
          |      foldEither(err -> Left(err), a -> f(a), fa)
          |   def map[A, B](f: Function[A, B], fa: Either[String, A]): Either[String, B] =
          |      foldEither(err -> Left(err), a -> Right(f(a)), fa)
          |}
          |
      ...
      compilerScan(
        ...
        """import eliot.lang.Function
          ...
          |
          |def graise[A](msg: String): Either[String, A] = raise(msg)
          |""".stripMargin
      )
  ...
  "the compiler track (Increment E — guard combinators)" should "reduce a generic guard's `raise` to the concrete `Either::raise`" in {
    refsOf("graise").asserting(_ shouldBe Seq(("Either", "raise")))
  }
}
