...
object ConstructorTypeAnalyzer {
  ...
  /** Count the number of fields a constructor has from its type signature. The type has the form: GenParam -> ... ->
    * Function^Type(Field1, Function^Type(Field2, ... DataType)). We skip leading FunctionLiterals (generic params) then
    * count Function^Type applications.
    */
