...
/** Generates the source code for a given File, i.e. reads it from disk. Note, that this generator does not fail if the
  ...
  *
  * Since CP1.5 every [[com.vanillasource.eliot.eliotc.source.scan.PathScan]] URI is a `file:` URI — the classpath scan
  * that produced `jar:`/resource URIs is gone, so all source content comes from [[FileContent]] on disk.
  */
