...
/** The effects-as-rows **elaboration phase** (`docs/effects-as-rows.md` §3, migration step R5): rewrites every demanded
  * value's direct-style runtime body into explicit monadic core via [[RowElaborator]], so the checker, monomorphization
  * and codegen all consume bodies whose `flatMap`/`pure`/`runId` are already written down.
  *
  * Placed after [[RecursionCheckedValue]] (the recursion gate walks the *user's* reference graph, before any machinery
  * call is spliced in) and before `SaturatedValueProcessor` (whose input is repointed to this fact), so everything from
  * saturation onwards sees elaborated bodies. A value that is not [[RowChecker.checkable]] — body-less, a `@Signature`
  * twin, a type constructor or meta companion — is carried through unchanged, as is a value whose body is already pure
  * (elaboration is the identity on pure code).
  *
  * **The universe is built by demand, not guessed.** Elaboration is decision-free but not context-free: it consults the
  * *declared* signature, effect row and slot modes of every callee it meets, plus one level of type alias inside those
  * signatures (the `=>` alias) and the registered run boundaries. Which names those are cannot be read off the body
  * alone — a slot's classification decides whether a further name is consulted at all. So this processor runs
  ...
  *   [[com.vanillasource.eliot.eliotc.row.RunBoundaryFunctions.configKey]]: a nominal-run return (`def main: IO[Unit]`)
  *   and a run-boundary argument are carrier regions, which is declared information only this registry holds.
  */
class RowElaborationProcessor(runBoundaryFunctions: Set[ValueFQN] = Set.empty)
  ...
  /** The per-definition **row verification** (`docs/effects-as-rows.md` §2/§5): `derived ⊆ declared`, checked on the
    * declared world before elaboration, located at the definition itself. A leak aborts this value — nothing downstream
    * (checker, mono, codegen) runs for it, so the user gets one located, effect-vocabulary error instead of the
    ...
    * the diagnostic the post-mono `DeclaredPureChecker` used to voice after a failed monomorphization.
    */
  ...
  /** The **rule-4 enforcement** (`docs/effects-as-rows.md` §1 rule 4, A.11.7-T step 2): a computation may not be
    ...
    *
    * The fix is always the same shape and the diagnostic says so: call the target **directly**, where its own declared
    * slot (pinned, carrier-typed, or row-declaring) already has the right mode. What the rule takes away is the *dot*
    * spelling of a discharge (`p.runStateToValue(s0)`), not the discharge — and in particular not the infix
  ...
  private def verifyRowlessSlots(elaboration: RowElaborator.Elaborated): CompilerIO[Unit] =
    elaboration.violations match {
      ...
      case violations =>
        // Each violation carries its own explanation: the two directions (a computation at a rowless slot, a plain
        // value at a carrier-typed one) read oppositely, so one shared help text would mislead half the time.
