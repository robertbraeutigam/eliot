...
object Qualifier {
  ...
  /** Function belongs to the given ability implementation.
    ...
    * its type-argument pattern plus its `where` guard, e.g. `Int[L1, H1], Int[L2, H2]` or `Cmin, Cmax where fitsByte…`.
    ...
    * two genuinely distinct instances of one ability in a module (e.g. `Eq[Type]` vs `Eq[String]`, or the guard-disjoint
    * `IntArith` width family) apart. The pattern is *also* recoverable from the synthetic marker function's signature;
