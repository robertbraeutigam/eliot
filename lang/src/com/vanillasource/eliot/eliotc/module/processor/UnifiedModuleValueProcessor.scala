...
class UnifiedModuleValueProcessor extends SingleFactProcessor[UnifiedModuleValue.Key] with Logging {
  ...
  /** Reconciles every co-located declaration of a name across the pool's layers into one value.
    *
    * The rules stay order-free with one exception, the compiler-as-platform override: among the *implementations*
    * (`runtime.isDefined`), an implementation that comes from an **override file** (the compiler overlay, per
    * [[PathScan.overrideFiles]]) supersedes the platform's — so the compiler track can redefine a name the platform also
    * implements without it counting as a conflict. Absent any override (always so on the runtime track, where
    * `overrideFiles` is empty), the ordinary rule applies: more than one implementation is a hard error. Every
