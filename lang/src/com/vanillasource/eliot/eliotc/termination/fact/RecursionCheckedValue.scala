...
/** An [[OperatorResolvedValue]] that has passed the no-recursion rule (termination M1): its body-level
  ...
  * (`OperatorResolvedValue` → `RecursionCheckedValue` → [[com.vanillasource.eliot.eliotc.effect.fact.EffectCheckedValue]]
  ...
  * trips `registerFactIfClear`, so a recursive value never produces this fact; because `EffectCheckProcessor`
  * reads it as its sole input, such a value never reaches the effect checks, saturation, monomorphization or codegen
