...
/** The structured **effect-row metadata** of a value's signature — the effects-as-channel "declared row + row
  ...
  *
  * **Dark / inert in Phase 1**: nothing reads this yet. It exists so Phase 2's shadow accounting can compute a derived
  * row and compare `derived ⊆ declared` against the current `EffectResidualChecker`. The carrier desugar still runs, so
  ...
  *
  * Only *open* rows populate it — a **pinned** row (`{Throw[E] | Id} A`) is an ordinary concrete carrier stack, not a
  * channel row, so it never appears here. Body rows are excluded too (the body is the runtime, not the declared
  * signature); so are generic-parameter-bound rows (they are neither the value's ambient nor a value parameter).
  *
  * @param returnEffects
...
case class EffectRow[C](
  ...
  def map[D](f: C => D): EffectRow[D] =
    EffectRow(returnEffects.map(f), parameterEffects.map(_.map(f)))
  ...
  def traverse[F[_]: Applicative, D](f: C => F[D]): F[EffectRow[D]] =
    (returnEffects.traverse(f), parameterEffects.traverse(_.traverse(f))).mapN(EffectRow(_, _))
}
