...
object TokenClassifier {
  case class AnnotatedPart(part: Sourced[OperatorResolvedExpression], fixity: Fixity, vfqn: Option[ValueFQN])

  sealed trait Token
  case class PrefixOp(part: Sourced[OperatorResolvedExpression])                                            extends Token
  case class PostfixOp(part: Sourced[OperatorResolvedExpression])                                           extends Token
  case class InfixOp(part: Sourced[OperatorResolvedExpression], associativity: Associativity, vfqn: ValueFQN) extends Token
  case class Operand(part: Sourced[OperatorResolvedExpression])                                             extends Token

  private case class ClassifyState(
      tokens: Vector[Token],
      operandGroup: Vector[Sourced[OperatorResolvedExpression]],
    ...
    def addOperand(part: Sourced[OperatorResolvedExpression], nowExpecting: Boolean): ClassifyState =
    ...
    tokens.foldLeft(Vector.empty[Token]) { (acc, token) =>
      (acc.lastOption, token) match {
        case (Some(Operand(operand)), PostfixOp(op)) =>
          acc.init :+ Operand(outlinedExpr(Seq(operand, op), OperatorResolvedExpression.FunctionApplication(op, operand)))
        case _                                       => acc :+ token
      }
    }
    ...
    tokens.foldRight(List.empty[Token]) { (token, acc) =>
      (token, acc) match {
        case (PrefixOp(op), Operand(operand) :: rest) =>
          Operand(outlinedExpr(Seq(op, operand), OperatorResolvedExpression.FunctionApplication(op, operand))) :: rest
        case _                                        => token :: acc
      }
    }
  ...
  def curriedApplication(
    ...
    parts.reduceLeft { (acc, arg) =>
      outlinedExpr(Seq(acc, arg), OperatorResolvedExpression.FunctionApplication(acc, arg))
    }
