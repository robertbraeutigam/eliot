...
/** Checks, for a given (module, ability-name) pair, that no two implementations of that ability
  * defined in that module have structurally overlapping patterns.
  *
  * Runs at definition time — as soon as any call site triggers ability resolution for this ability —
  * so overlap errors point at the offending implementations rather than at call sites that merely
  * expose the ambiguity.
  *
  * Cross-module overlap is still detected at call time by the ambiguity check in
  * `AbilityImplementationProcessor`: the definition-time check here can only see impls in one module
  * at a time, whereas impls can legitimately live in either the ability's module or a type's module.
  */
class ModuleAbilityOverlapCheckProcessor extends SingleKeyTypeProcessor[ModuleAbilityOverlapCheck.Key] {

  override protected def generateFact(key: ModuleAbilityOverlapCheck.Key): CompilerIO[Unit] =
    for {
      names      <- getFactOrAbort(UnifiedModuleNames.Key(key.moduleName))
      // Find every marker function for this ability in this module. The marker's local name equals
      // the ability name and its qualifier carries the impl's index, which uniquely identifies
      // the implementation.
      markers     = names.names.keys.toSeq.collect {
      ...
      withVfqns   = markers.zip(patterns).collect { case (vfqn, Some(p)) => (vfqn, p) }
      _          <- reportOverlaps(withVfqns)
      _          <- registerFactIfClear(ModuleAbilityOverlapCheck(key.moduleName, key.abilityName))
    } yield ()

  /** Returns `(typeVars, argTypes)` for the marker function, or `None` if its signature can't be
    * evaluated (e.g. because it transitively refers to an abstract or erroring value).
    */
