...
/** An [[OperatorResolvedValue]] whose runtime body has had every `namedValues[V](name)` call expanded into its residual
  * builder chain ([[com.vanillasource.eliot.eliotc.namedvalues.processor.NamedValuesRewriteProcessor]]). Sits between
  * the `operator` and `termination` phases of the value chain (`OperatorResolvedValue` → `NamedValuesRewrittenValue` →
...
object NamedValuesRewrittenValue {
  case class Key(vfqn: ValueFQN, platform: Platform = Platform.Runtime)
    ...
    override def valueCodec: Option[FactCodec[NamedValuesRewrittenValue]] = Some(
      LangFactCodecs.namedValuesRewrittenValueCodec
    )
  }
}
