...
/** Shared base for the two *user-category* body suppliers — [[UserValueNativesProcessor]] (the runtime layer) and
  ...
  *
  * Crucially these suppliers do **not** resolve dependencies: they read only [[SaturatedValue]], never
  * [[NativeBinding]]. The transitive dependency closure is done by the [[BindingMergerProcessor]] for the *selected*
  * contribution, so a supplier never reads back the merged fact it feeds — the [[NativeBinding]] recursion stays the
  * merger's single, self-contained concern.
  */
