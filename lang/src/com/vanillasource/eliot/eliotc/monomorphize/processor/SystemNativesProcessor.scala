...
/** The `system` native contributor: emits the total [[ContributedBinding]] under [[ContributedBinding.systemLabel]] for
  ...
  * here rather than in the library layer. The comparison itself is [[ConcreteNormalForm]], shared with the effect
  * intrinsics' frame keys. The `implement Eq[Type]` method is body-less, so — unlike the other system
...
class SystemNativesProcessor extends SingleFactProcessor[ContributedBinding.Key] {
  ...
  private def systemReduction(vfqn: ValueFQN): Option[SemValue] =
    ...
    else ListReductions.bindings.get(vfqn).orElse(EffectIntrinsics.bindings.get(vfqn))
  ...
  /** `integerLiteral[V]: Int` — the value-position literal protocol. A value-position literal `n` is desugared to
    ...
    *
    * But when the literal is *consumed by a compile-time computation* — a `where` guard bound (e.g. `withinByte`, which
    * compares an interval's endpoints against the `byteMin`/`byteMax` bounds), or
  ...
  private def typeEqualsResult(implFqn: ValueFQN, a: SemValue, b: SemValue): SemValue =
    if (ConcreteNormalForm.isConcrete(a) && ConcreteNormalForm.isConcrete(b))
      (if (ConcreteNormalForm.equal(a, b)) Evaluator.trueValue else Evaluator.falseValue)
