...
/** The `compiler` native supplier: emits the total [[ContributedBinding]] under [[ContributedBinding.compilerLabel]] for
  ...
  * **compiler** marker exactly what [[UserValueNativesProcessor]] is to the **runtime** marker — it builds the
  * `VTopDef` binding from a value's own (checking) body — with two differences: it reads the
  * [[Platform.Compiler]]-marker [[SaturatedValue]] (so it sees the compiler platform's redefinition, e.g. the
  * compile-time `Either` carrier, not the runtime layer's), and it is registered as a **native** label, so its
  ...
  * [[SaturatedValue]] request. This avoids the `UnifiedModuleValueProcessor` "Could not find" abort for a name present
  * in the runtime pool but absent from a shared compiler-pool module; a module absent from the compiler path entirely
  * aborts silently in `PathScanner` (the compiler path is an overlay, not the build), so its `UnifiedModuleNames` is
  * simply unavailable and membership is `false`.
  *
  ...
  */
class CompilerNativesProcessor
    extends BindingProcessor[ContributedBinding.Key](key => SaturatedValue.Key(key.vfqn, Platform.Compiler)) {

  // Only this label's queries belong to the compiler contributor; short-circuit other labels before any fetch. A
  // member of the compiler pool is built by the shared BindingProcessor from its compiler-marker SaturatedValue; a
  ...
  // Mirrors UserValueNativesProcessor: a body-less compiler-pool value (an abstract base declaration) has no
  // compiler-platform reduction here — its checking implementation is a native or a runtime-only stuck fallback — so it
  // contributes `None`, never an empty user-style VTopDef that would shadow a reducing native.
  ...
  override protected def buildFact(vfqn: ValueFQN, binding: Option[SemValue]): CompilerIO[ContributedBinding] =
    ContributedBinding(vfqn, ContributedBinding.compilerLabel, binding).pure[CompilerIO]

  /** Whether `vfqn` is declared by its module in the compiler pool. Reading [[UnifiedModuleNames]] (a name set) rather
    * than [[SaturatedValue]] keeps the non-member case quiet: an absent module yields no fact (the compiler-path miss
    * aborts silently in `PathScanner`) and a present module simply does not list the name — neither path emits the
    * "Could not find" error a direct value request would.
    */
