...
/** Quiet two-pool membership probe for the leaf native contributors ([[DataTypeNativesProcessor]],
  ...
  * contributor must read a *platform-keyed* source fact (the value's
  * [[com.vanillasource.eliot.eliotc.operator.fact. OperatorResolvedValue]], its
  * [[com.vanillasource.eliot.eliotc.module.fact.ModuleConstructors]]) to build a reduction. Requesting that value on a
  * fixed pool a name is absent from would trip the `UnifiedModuleValueProcessor` "Could not find" error as a build side
  * effect — exactly the hazard [[CompilerNativesProcessor.inCompilerPool]] guards against.
  *
