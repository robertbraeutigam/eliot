...
- It ships only `.els` sources under `eliot/…`; it carries no Scala processors (CP3's
  `CompilerNativesProcessor` lives in `lang`).
- Its `eliot` dir is listed on the **`--compiler-path`** of every type-checking entry point (the
