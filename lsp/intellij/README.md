...
The Eliot compiler doubles as a language server (`lsp` module, `LspMain`). It speaks LSP over
...
This produces `lsp/dist/` (git-ignored):

- `lsp/dist/eliot-lsp` — the launcher script (this is the command IntelliJ runs).
- `lsp/dist/lib/` — the per-module + dependency jars.
- `lsp/dist/lsp4ij-template/template.json` — a ready-to-import LSP4IJ template with the **absolute**
  launcher path already filled in for this checkout.

> **Do not** use `mill lsp.assembly` (a fat jar). Eliot's platform layers keep multiple files at the
...
Re-run `./lsp/package.sh` after changing compiler/server code to refresh the jars.
...
2. **Server** tab:
   - **Name:** `Eliot`
   - **Command:** `sh -c "/absolute/path/to/eliot/lsp/dist/eliot-lsp"`
     (on Windows use `cmd /c "...\lsp\dist\eliot-lsp"` with an equivalent `.bat`, or run via WSL).
...
- **Everything shows "Name not defined" (e.g. `println`)** — you're almost certainly launching a fat
  assembly jar instead of `lsp/dist/eliot-lsp`. Use the launcher; see the warning in step 1.
- **Server doesn't start** — run `lsp/dist/eliot-lsp` in a terminal; it should block waiting for LSP
