...
class UnifiedModuleValueProcessorTest
  ...
  "unified module value processor" should "create unified value for a simple constant" in {
    runEngineForValue("a: A", "a").asserting(_.vfqn shouldBe ValueFQN(testModuleName2, QualifiedName("a", Qualifier.Default)))
  }

  it should "prefer implemented value over abstract" in {
    runEngineForValue("a: A = b", "a").asserting(_.namedValue.runtime.isDefined shouldBe true)
  }
  ...
  private def runEngineForValue(source: String, name: String): IO[UnifiedModuleValue] =
    runGenerator(source, UnifiedModuleValue.Key(ValueFQN(testModuleName2, QualifiedName(name, Qualifier.Default)))).map { case (_, facts) =>
      facts.values.collectFirst { case uv: UnifiedModuleValue if uv.vfqn.name == QualifiedName(name, Qualifier.Default) => uv }.get
    }

  private def runEngineForErrors(source: String, name: String): IO[Seq[String]] =
    runGenerator(source, UnifiedModuleValue.Key(ValueFQN(testModuleName2, QualifiedName(name, Qualifier.Default)))).map(_._1.map(_.message))
}
