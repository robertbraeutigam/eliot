...
class DesugarProcessor
    extends OneToOneProcessor((key: DesugaredSourceAST.Key) => UnifiedSourceAST.Key(key.path))
    with Logging {

  override def generateFromFact(sourceAst: UnifiedSourceAST)(using process: CompilationProcess): IO[Unit] =
    process.registerFact(
      DesugaredSourceAST(sourceAst.path, sourceAst.sourcedAst.as(desugar(sourceAst.sourcedAst.value)))
    )
