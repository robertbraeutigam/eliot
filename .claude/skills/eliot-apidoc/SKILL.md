...
3. **Give an example when it earns its place** — operators, combinators, effectful APIs, or any non-obvious behaviour. Use a runnable-looking fenced `eliot` block; keep it minimal. Skip examples for self-explanatory definitions.
...
- Examples (if any) are in fenced `eliot` blocks and minimal.
