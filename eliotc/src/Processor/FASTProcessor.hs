...
parseFASTProcessor (FunctionCompilationUnit fname _ (FunctionDefinition _ _ ([PositionedToken _ _ _ (Keyword "native")]))) =
   registerCompilerFact (CompiledFunctionSignal fname) (CompiledFunction fname Nothing)
parseFASTProcessor (FunctionCompilationUnit fname dictionary (FunctionDefinition _ _ [calledToken])) =
...
parseFASTProcessor (FunctionCompilationUnit _ _ (FunctionDefinition _ _ ts)) = compilerErrorForTokens ts "Function body must be only one function name (Under development)."
