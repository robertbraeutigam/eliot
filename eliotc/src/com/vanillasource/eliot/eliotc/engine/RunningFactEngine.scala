...
import com.vanillasource.eliot.eliotc.CompilerFactKey
...
case class RunningFactEngine[K, V] private (
    ...
    status: RunningFactEngineStatus[K],
    facts: STMMap[K, V]
)(using
  ...
  def getFact(key: K): IO[Option[V]] = status.wrapLookup(key, lookupFact(key).commit)
  ...
  private def lookupFact(key: K): STM[Option[V]] = for {
    ...
    facts       <- facts.toMap
    stalled     <- status.stalled(facts.keySet)
    _           <- valueOption match
...
object RunningFactEngine {
  private[engine] def create[K, V](processors: Seq[FactProcessor[K, V]]): IO[RunningFactEngine[K, V]] =
    for {
      ...
      status     <- RunningFactEngineStatus.initialStatus[K](using stmRuntime)
