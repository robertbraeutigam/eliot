...
import com.vanillasource.eliot.eliotc.ast.TokenParser.{Success, NoSuccess, astParser, tokenShow}
  ...
  private def parseAST(file: File, tokens: Seq[Sourced[Token]])(using process: CompilationProcess): IO[Unit] =
    astParser.apply(TokenStream(tokens)): @unchecked match {
      ...
      case NoSuccess(err, nextTokens) =>
        compilerError(file, nextTokens.first.map(_ => s"Expected $err, but encountered ${nextTokens.first.value.show}"))
    }
}
