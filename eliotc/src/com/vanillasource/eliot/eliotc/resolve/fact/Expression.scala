...
object Expression {
  ...
  case class FunctionLiteral(parameter: ArgumentDefinition, body: Expression)                    extends Expression
  ...
  given Show[Expression] = {
    ...
    case FunctionLiteral(parameter, body)                       => parameter.name.value + " -> " + body.show
