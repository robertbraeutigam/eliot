...
/** The cache data the generator produces and consumes: one [[CacheEntry]] per fact reachable from the build target.
  ...
  * running compiler's whole classpath, so any such change already discards the cache. The fingerprints that guard reuse
  * live in the on-disk header, not here (see [[ContentAddressedCacheBackend]]).
  */
