...
class ParserTest extends AnyFlatSpec with Matchers {
  "end-of-input parser" should "match end-of-input" in {
    ...
    p.runParser("") shouldBe Success(consumed = false, ParserError.noError, ())
  }

  it should "return error if stream did not reach end-of-input" in {
    val p = endOfInput[Char]()

    p.runParser("abc") shouldBe Failure(consumed = false, ParserError(0, Seq("end of input")))
  }

  "literal" should "match the literal in stream" in {
    val p = literal('a')

    p.runParser("abc") shouldBe Success(consumed = true, ParserError.noError, 'a')
  }

  it should "fail on different literal" in {
    val p = literal('a')

    p.runParser("bc") shouldBe Failure(consumed = false, ParserError(0, Seq("a")))
  }

  "full parsing" should "succeeds if stream ends" in {
    val p = literal('a').fully()

    p.runParser("a") shouldBe Success(consumed = true, ParserError.noError, 'a')
  }

  it should "fail if stream does not end" in {
    val p = literal('a').fully()

    p.runParser("ab") shouldBe Failure(consumed = true, ParserError(1, Seq("end of input")))
  }

  "optional parser" should "parse if successful" in {
    val p = literal('a').optional()

    p.runParser("a") shouldBe Success(consumed = true, ParserError.noError, Some('a'))
  }

  it should "return none successfully if parser fails" in {
    val p = literal('a').optional()

    p.runParser("b") shouldBe Success(consumed = false, ParserError(0, Seq("a")), None)
  }

  "sequence of parsers" should "return skip if first parser fails" in {
    val p = literal('a') >> literal('b')

    p.runParser("ab") shouldBe Success(consumed = true, ParserError.noError, 'b')
  }

  it should "indicate nothing consumed if first parser fails" in {
    val p = literal('a') >> literal('b')

    p.runParser("bb") shouldBe Failure(consumed = false, ParserError(0, Seq("a")))
  }

  it should "indicate something consumed if second parser fails" in {
    val p = literal('a') >> literal('b')

    p.runParser("ac") shouldBe Failure(consumed = true, ParserError(1, Seq("b")))
  }

  "expected items" should "be listed in a many followed by a literal" in {
    val p = literal('a').anyTimes() >> literal('b')

    p.runParser("aac") shouldBe Failure(consumed = true, ParserError(2, Seq("a", "b")))
  }
  ...
  it should "return second parser, if the first does not match" in {
    ...
    p.runParser("b") shouldBe Success(consumed = true, ParserError.noError, 'b')
  }

  it should "return both expected if none match" in {
    val p = literal('a').or(literal('b'))

    p.runParser("c") shouldBe Failure(false, ParserError(0, Seq("a", "b")))
  }
  ...
  it should "collect all found matches if it can match any times" in {
    ...
    p.runParser("..abc..abd..aaabc..") shouldBe Success(consumed = true, ParserError(0, Seq("a")), 2)
  }
  ...
  it should "collect all found matches, and then continue to parse after last match if combined with atomic and any times" in {
    ...
    p.runParser("..abc..abccd") shouldBe Success(consumed = true, ParserError.noError, 99)
  }
  ...
  it should "report expected from both itself and following parser after last match" in {
    ...
    p.runParser("..abc..abcxcd") shouldBe Failure(consumed = true, ParserError(10, Seq("a", "c")))
  }
  ...
  it should "parse success even if nothing found with anyTimes()" in {
    ...
    p.runParser("..abd..abd..aaabd..") shouldBe Success(consumed = false, ParserError(0, Seq("a")), 0)
  }
