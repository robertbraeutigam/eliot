...
object HtmlSite {
  ...
  private def page(title: String, sidebar: String, content: String): String =
    s"""<!doctype html>
       ...
       |  document.querySelectorAll('#tree .tree-leaf').forEach(function(li){
       |    li.style.display=(q===''||li.getAttribute('data-name').toLowerCase().indexOf(q)>=0)?'':'none';
       |  });
       |  Array.prototype.slice.call(document.querySelectorAll('#tree li.tree-dir')).reverse().forEach(function(li){
       |    var any=false;
       |    li.querySelectorAll('.tree-leaf').forEach(function(c){if(c.style.display!=='none')any=true;});
       |    li.style.display=(q===''||any)?'':'none';
       |    var d=li.querySelector('details');
       |    if(d){d.open=(q===''?true:any);}
       |  });
       |}
       |</script>
       |</body>
