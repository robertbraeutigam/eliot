...
/** Discovers the Eliot source roots beneath the editor's workspace folders.
  ...
  * roots, and neither LSP4IJ nor a Mill/BSP import marks them. So we recover the roots from each workspace folder by
  * the build system's standard layout:
  *
  *   - a directory named `src` or `test` with `.els` files beneath it is a package's source root — module
  *     `eliot.lang.X` lives at `<root>/eliot/lang/X.els`; the `compiler/` sibling of a `src` (its compile-time
  *     overlay) is added by `LangPlugin`;
  *   - any other directory that *directly* contains `.els` files is a root too — module `X` at `<root>/X.els` — which
  *     is what reaches a tree laid out by nobody's convention.
  *
  * A discovered root is not descended into (so the `eliot` package dir *inside* a root is not mistaken for a second
  * root), and build/hidden/`compiler` directories are skipped. A `src` holding another language's sources (every
  * Mill-style Scala module in this repo) has no `.els` beneath it: it is neither a root nor descended into.
  *
...
object SourceRootDiscovery {
  private val sourceRootNames       = Set("src", "test")
  private val overlayDirectoryName  = "compiler"
  ...
  private def rootsUnder(workspaceRoot: Path): Seq[Path] = {
    ...
    def visit(directory: Path): Unit =
      ...
      else if (sourceRootNames.contains(directory.getFileName.toString)) {
        if (hasEliotSourceBeneath(directory)) roots += directory // package source root; a `src` with no Eliot sources
        // (a Scala module's) is a dead branch — neither a root nor descended into.
      } else if (directlyContainsEliotSource(directory)) roots += directory // a root by its contents alone
      else childDirectories(directory).foreach(visit)
    // The workspace folder is the project, not a source root, so never apply the name rule to it.
