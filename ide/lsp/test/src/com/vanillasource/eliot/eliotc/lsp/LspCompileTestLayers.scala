...
/** Shared helper for the LSP *compile* tests. A real compile needs the abstract base and the `jvm` layer on the path,
  ...
  * via the `ELIOT_REPO_ROOT` env var (see `build.mill`); each module's `eliot/src` source root is resolved under it.
  */
object LspCompileTestLayers {
  private val repoRoot                         =
    Path.of(Option(System.getenv("ELIOT_REPO_ROOT")).getOrElse(System.getProperty("user.dir")))
  private def moduleRoot(module: String): Path = repoRoot.resolve(module).resolve("eliot").resolve("src")

  private val layerPaths: Seq[Path] =
    Seq(moduleRoot("lang"), moduleRoot("stdlib"), moduleRoot("jvm"))

  /** Add the base + `jvm` layer `eliot/src` source roots to a test configuration, *appending* to any workspace roots
    * the test already put in `LangPlugin.pathKey`. Each root's sibling `compiler/` overlay (only `stdlib` ships one) is
    * added to the compile-time pool by `LangPlugin`.
    */
