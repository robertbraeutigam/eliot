...
/** End-to-end proof that a standalone project — sources directly under `src/`, with no `compiler/` sibling of its own
  * — compiles cleanly when its only knowledge of the base/stdlib/platform layers is an
...
class WorkspacePathsCompileTest extends AsyncFlatSpec with AsyncIOSpec with Matchers {
  ...
  private def layer(module: String): Path = repoRoot.resolve(module).resolve("eliot").resolve("src")
  ...
  private val eliotPaths = Seq(
    ...
    s"runtime  ${layer("jvm")}",
    s"# this project's own source root",
    s"runtime  src",
    s"# the stdlib compile-time overlay, listed separately",
    s"compiler ${repoRoot.resolve("stdlib").resolve("eliot").resolve("compiler")}"
  ).mkString("\n")

  "a project configured by eliot.paths" should "compile with no errors, resolving the layers it lists" in {
    tempDirectory
      .use { workspace =>
