...
serialization outright — and **§14 measures its first step**: coverage is compiler-proven (39 of 41 fact types, the
2 declines exactly as predicted), and the encoding lands at **0.55× the Java graph with sharing, 4.89× without** —
which confirms the design and shows the two halves cannot ship separately.
