...
**The front-end is the natural incremental boundary, fundamentally.** The native-implementation facts (`NativeBinding`, and the natives `TransparentBinding` builds on) carry a `SemValue` whose `VNative.fire` field *is* a Scala closure — the actual JVM intrinsic / compile-time evaluation logic (arithmetic, `&&`, `fold`, pattern-match dispatch). That is **code, not data**: it can never be serialized, and there is no "ground form" to convert it into. Every monomorphize computation is anchored to these closures, so the whole monomorphize phase necessarily regenerates each run. This is not a serialization gap to be closed — it is a property of the architecture. The real, durable incremental savings are therefore in the front-end (skip re-reading/-tokenizing/-parsing/-resolving unchanged files) plus the no-change codegen accept.

(The only conceivable way to cache monomorphize would be to stop comparing native bindings as values and treat them as **version-stable constants** — they change only when the compiler changes, which `CACHE_VERSION` already invalidates — so dependents could cache. That is a separate, larger effort, and would additionally require the monomorphize facts above the natives to be ground/serializable. Out of scope; the drop-and-regenerate behavior here is correct and safe regardless.)
